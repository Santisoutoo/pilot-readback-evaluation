...
class Finding(BaseModel):
    ...
    element: str = Field(
        description="Element judged, e.g. 'callsign', 'squawk', 'phraseology'.")
    expected: Optional[str] = Field(
        default=None, description="Expected value, if applicable.")
    got: Optional[str] = Field(
        default=None, description="Value found in the readback, if applicable.")
    verdict: Literal["good", "bad"] = Field(
        description="Whether this element was correct.")
...
class CategoryScore(BaseModel):
    ...
    score: int = Field(
        ge=0, le=10, description="0-10 integer score for this category.")
...
class TurnEvaluation(BaseModel):
    ...
    overall_score: int = Field(
        ge=0, le=100, description="0-100 integer overall score.")
